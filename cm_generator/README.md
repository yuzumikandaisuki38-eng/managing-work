...
携帯電話から同じWi-Fi経由で使う場合は、パソコンで次のように起動します。

```bash
python3 cm_generator/local_server.py --host 0.0.0.0
```

パソコンのローカルIP（例: `192.168.1.10:8000`）を、サイトの「生成サーバーURL」に
入力してください。インターネットへ公開する場合は、この開発用サーバーを直接公開せず、
認証付きの本番サーバーを利用してください。
